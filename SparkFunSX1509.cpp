...
void SX1509::pinMode(byte pin, byte inOut) { pinDir(pin, inOut); }

void SX1509::writePin(byte pin, byte highLow) {
  unsigned int tempRegDir = readWord(REG_DIR_B);

  if ((0xFFFF ^ tempRegDir) &
      (1 << pin))  // If the pin is an output, write high/low
  {
...
byte SX1509::readPin(byte pin) {
  ...
  {
    unsigned int tempRegData = readWord(REG_DATA_B);
    if (tempRegData & (1 << pin)) return 1;
  }

  return 0;
}

byte SX1509::digitalRead(byte pin) { return readPin(pin); }
...
void SX1509::analogWrite(byte pin, byte iOn) { pwm(pin, iOn); }

void SX1509::blink(byte pin, unsigned long tOn, unsigned long tOff,
...
void SX1509::breathe(byte pin, unsigned long tOn, unsigned long tOff,
                     unsigned long rise, unsigned long fall, byte onInt,
                     byte offInt, bool log) {
...
void SX1509::setupBlink(byte pin, byte tOn, byte tOff, byte onIntensity,
                        byte offIntensity, byte tRise, byte tFall, bool log) {
  ...
  if (REG_T_RISE[pin] != 0xFF) writeByte(REG_T_RISE[pin], tRise);
  // Write regTFall
  // 0: off
  // 1-15:  TFall =      (regIOn - (4 * offIntensity)) * tFall * (255/ClkX)
  // 16-31: TFall = 16 * (regIOn - (4 * offIntensity)) * tFall * (255/ClkX)
  if (REG_T_FALL[pin] != 0xFF) writeByte(REG_T_FALL[pin], tFall);
}

void SX1509::keypad(byte rows, byte columns, unsigned int sleepTime,
                    byte scanTime, byte debounceTime) {
  unsigned int tempWord;
  byte tempByte;

  // If clock hasn't been set up, set it to internal 2MHz
  if (_clkX == 0) clock(INTERNAL_CLOCK_2MHZ);

  // Set regDir 0:7 outputs, 8:15 inputs:
  tempWord = readWord(REG_DIR_B);
  for (int i = 0; i < rows; i++) tempWord &= ~(1 << i);
  for (int i = 8; i < (columns * 2); i++) tempWord |= (1 << i);
  writeWord(REG_DIR_B, tempWord);

  // Set regOpenDrain on 0:7:
  tempByte = readByte(REG_OPEN_DRAIN_A);
  for (int i = 0; i < rows; i++) tempByte |= (1 << i);
  writeByte(REG_OPEN_DRAIN_A, tempByte);

  // Set regPullUp on 8:15:
  tempByte = readByte(REG_PULL_UP_B);
  for (int i = 0; i < columns; i++) tempByte |= (1 << i);
  ...
  if (debounceTime >= scanTime) {
    debounceTime =
        scanTime >> 1;  // Force debounceTime to be less than scanTime
  }
  ...
  if (sleepTime != 0) {
    ...
    if (sleepTimeBits == 0) sleepTimeBits = 1;
  }
...
unsigned int SX1509::readKeypad() { return readKeyData(); }
...
void SX1509::debouncePin(byte pin) { debounceEnable(pin); }
...
void SX1509::debounceKeypad(byte time, byte numRows, byte numCols) {
  ...
  for (int i = 0; i < numRows; i++) debouncePin(i);
  for (int i = 0; i < (8 + numCols); i++) debouncePin(i);
}
...
unsigned int SX1509::interruptSource(bool clear /* =true*/) {
  ...
  if (clear) writeWord(REG_INTERRUPT_SOURCE_B, 0xFFFF);  // Clear interrupts
  return intSource;
}
...
bool SX1509::checkInterrupt(int pin) {
  if (interruptSource(false) & (1 << pin)) return true;

  return false;
}

void SX1509::clock(byte oscSource, byte oscDivider, byte oscPinFunction,
                   byte oscFreqOut) {
  configClock(oscSource, oscPinFunction, oscFreqOut, oscDivider);
}

void SX1509::configClock(byte oscSource /*= 2*/, byte oscPinFunction /*= 0*/,
...
byte SX1509::calculateSlopeRegister(int ms, byte onIntensity,
                                    byte offIntensity) {
  ...
  float tFactor =
      ((float)onIntensity - (4.0 * (float)offIntensity)) * 255.0 / (float)_clkX;
...
//	This function reads a single byte located at the registerAddress
// register.
...
//	This function writes a two-byte word to registerAddress and
// registerAddress + 1
