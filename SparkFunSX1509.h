...
class SX1509 {
 ...
 public:
  ...
  // Inputs:
  //		- address: should be the 7-bit address of the SX1509. This
  // should be 		 one of four values - 0x3E, 0x3F, 0x70, 0x71 - all
  // depending on what the 		 ADDR0 and ADDR1 pins ar se to. This
  // variable is required.
  //		- resetPin: This is the Arduino pin tied to the SX1509 RST pin.
  // This 		 pin is optional. If not declared, the library will
  ...
  //		reset or software. A hardware reset (hardware parameter = 1)
  // pulls the 		reset line low, pausing, then pulling the reset line
  // high. A software 		reset writes a 0x12 then 0x34 to the REG_RESET
  // as outlined in the 		datasheet.
  //
  //  Input:
  //	 	- hardware: 0 executes a software reset, 1 executes a hardware
  // reset
  // -----------------------------------------------------------------------------
  ...
  //	 	- inOut: The Arduino INPUT and OUTPUT constants should be used
  // for the 		 inOut parameter. They do what they say!
  // -----------------------------------------------------------------------------
  void pinMode(byte pin, byte inOut);
  void pinDir(byte pin, byte inOut);  // Legacy - use pinMode

  // -----------------------------------------------------------------------------
  // digitalWrite(byte pin, byte highLow): This function writes a pin to either
  // high
  //		or low if it's configured as an OUTPUT. If the pin is configured
  // as
  // an 		INPUT, this method will activate either the PULL-UP
  // or PULL-DOWN 		resistor (HIGH or LOW respectively).
  //
  //	Inputs:
  //		- pin: The SX1509 pin number. Should be a value between 0
  // and 15.
  ...
  //		The pin should be configured as an INPUT, using the pinDir
  // function.
  //
  //	Inputs:
  //	 	- pin: The SX1509 pin to be read. should be a value between 0
  // and 15.
  //  Outputs:
  ...
  //		driving on a pin. It must be called if you want to use the pwm
  // or blink 		functions on that pin.
  //
  //	Inputs:
  //		- pin: The SX1509 pin connected to an LED. Should be 0-15.
  //   	- freq: Sets LED clock frequency divider.
  //		- log: selects either linear or logarithmic mode on the LED
  // drivers
  ...
  // -----------------------------------------------------------------------------
  // analogWrite(byte pin, byte iOn):	This function can be used to control the
  // intensity
  ...
  // -----------------------------------------------------------------------------
  // setupBlink(byte pin, byte tOn, byte tOff, byte offIntensity, byte tRise,
  // byte
  //		tFall):  blink performs both the blink and breath LED driver
  // functions.
  //
  // 	Inputs:
  ...
  //			- This value should be between 0 and 7. 0 is completely
  // off.
  ...
  //			- This value should be between 1 and 31. 0 is off.
  // 	 Notes:
  //		- The breathable pins are 4, 5, 6, 7, 12, 13, 14, 15 only. If
  // tRise and 			tFall are set on 0-3 or 8-11 those pins will
  // still only blink.
  // 		- ledDriverInit should be called on the pin to be blinked before
  // this.
  // -----------------------------------------------------------------------------
  void setupBlink(byte pin, byte tOn, byte toff, byte onIntensity = 255,
                  byte offIntensity = 0, byte tRise = 0, byte tFall = 0,
                  bool log = false);

  // -----------------------------------------------------------------------------
  // blink(byte pin, unsigned long tOn, unsigned long tOff, byte onIntensity,
  // byte offIntensity);
  //  	Set a pin to blink output for estimated on/off millisecond durations.
  //
  // 	Inputs:
  //  	- pin: the SX1509 pin (0-15) you want to set blinking
  //   	- tOn: estimated number of milliseconds the pin is LOW (LED sinking
  //   current will be on)
  //   	- tOff: estimated number of milliseconds the pin is HIGH (LED sinking
  //   current will be off)
  ...
  // this.
  // -----------------------------------------------------------------------------
  void blink(byte pin, unsigned long tOn, unsigned long tOff,
             byte onIntensity = 255, byte offIntensity = 0);

  // -----------------------------------------------------------------------------
  // breathe(byte pin, unsigned long tOn, unsigned long tOff, unsigned long
  // rise, unsigned long fall, byte onInt, byte offInt, bool log);
  //  	Set a pin to breathe output for estimated on/off millisecond durations,
  //  with 	estimated rise and fall durations.
  //
  // 	Inputs:
  //  	- pin: the SX1509 pin (0-15) you want to set blinking
  //   	- tOn: estimated number of milliseconds the pin is LOW (LED sinking
  ...
  // -----------------------------------------------------------------------------
  void breathe(byte pin, unsigned long tOn, unsigned long tOff,
               unsigned long rise, unsigned long fall, byte onInt = 255,
               byte offInt = 0, bool log = LINEAR);

  // -----------------------------------------------------------------------------
  // keypad(byte rows, byte columns, byte sleepTime, byte scanTime, byte
  // debounceTime)
  //		Initializes the keypad function on the SX1509. Millisecond
  // durations for sleep, 		scan, and debounce can be set.
  //
  //	Inputs:
  //		- rows: The number of rows in the button matrix.
  //			- This value must be between 1 and 7. 0 will turn it
  // off.
  ...
  //		- scanTime: Sets the scan time per row. Must be set above
  // debounce.
  //  	  Should be a millisecond duration between 1 and 128.
  //   	  Possible values are 1, 2, 4, 8, 16, 32, 64, 128.
  //		- debounceTime: Sets the debounc time per button. Must be set
  // below scan.
  //  	  Should be a millisecond duration between 0 and 64.
  //   	  Possible values are 0 (0.5), 1, 2, 4, 8, 16, 32, 64.
  // -----------------------------------------------------------------------------
  void keypad(byte rows, byte columns, unsigned int sleepTime = 0,
              byte scanTime = 1, byte debounceTime = 0);
  ...
  //
  //	Output:
  //		A 16-bit value is returned. The lower 8 bits represent the up-to
  // 8
  // rows, 		while the MSB represents the up-to 8 columns. Bit-values
  // of 1 indicate a 		button in that row or column is being pressed.
  // As such, at least two 		bits should be set.
  // -----------------------------------------------------------------------------
  unsigned int readKeypad();
  unsigned int readKeyData();  // Legacy: use readKeypad();

  // -----------------------------------------------------------------------------
  // getRow(): This function returns the first active row from the return value
  // of
  //  	readKeypad().
  //
  //	Input:
  //      - keyData: Should be the unsigned int value returned from
  //      readKeypad().
  //	Output:
  //		A 16-bit value is returned. The lower 8 bits represent the up-to
  // 8
  ...
  // -----------------------------------------------------------------------------
  // getCol(): This function returns the first active column from the return
  // value of
  //  	readKeypad().
  //
  //	Input:
  ...
  //		blinking LEDs. Bit 2 of REG_MISC is set, which alters the
  // functionality 		of the nReset pin. The nReset pin is toggled
  // low->high, which
  // should 		reset all LED counters. Bit 2 of REG_MISC is again
  // cleared, returning 		nReset pin to POR functionality
  // -----------------------------------------------------------------------------
  void sync(void);

  // -----------------------------------------------------------------------------
  // debounceConfig(byte configValue): This method configures the debounce time
  // of
  //		every input.
  //
  //	Input:
  ...
  //
  //	Input:
  //		- time: A millisecond duration estimating the debounce time.
  // Actual 		  debounce time will depend on fOSC. Assuming it's 2MHz,
  // debounce will 		  be set to the 0.5, 1, 2, 4, 8, 16, 32, or 64
  // ms (whatever's closest)
  //
  //	Note: fOSC is set with the configClock function. It defaults to 2MHz.
  // -----------------------------------------------------------------------------
  ...
  //
  //	Input:
  //		- pin: The SX1509 pin to be debounced. Should be between 0
  // and 15.
  // -----------------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------------
  // debounceKeypad(byte pin): This method enables debounce on all pins
  // connected
  //  to a row/column keypad matrix.
  //
  //	Input:
  //		- time: Millisecond time estimate for debounce (see
  // debounceTime()).
  ...
  // -----------------------------------------------------------------------------
  // enableInterrupt(byte pin, byte riseFall): This function sets up an
  // interrupt
  //		on a pin. Interrupts can occur on all SX1509 pins, and can be
  // generated 		on rising, falling, or both.
  //
  //	Inputs:
  //		-pin: SX1509 input pin that will generate an input. Should be
  // 0-15. 		-riseFall: Configures if you want an interrupt generated
  // on rise fall
  // or 			both. For this param, send the pin-change values
  // previously defined 			by Arduino:
  // #define CHANGE 1
  // <-Both 			#define FALLING 2	<- Falling
  // #define
  // RISING 3	<- Rising
  //
  ...
  // -----------------------------------------------------------------------------
  // interruptSource(void): Returns an unsigned int representing which pin
  // caused
  //		an interrupt.
  //
  //	Output: 16-bit value, with a single bit set representing the pin(s) that
  //		generated an interrupt. E.g. a return value of	0x0104 would
  // mean pins 8 		and 3 (bits 8 and 3) have generated an
  // interrupt.
  //  Input:
  ...
  //
  //	Output: Boolean value. True if the requested pin has triggered an
  // interrupt/
  //  Input:
  ...
  // -----------------------------------------------------------------------------
  // configClock(byte oscSource, byte oscPinFunction, byte oscFreqOut, byte
  // oscDivider)
  //		This function configures the oscillator source/speed
  //		and the clock, which is used to drive LEDs and time debounces.
  //
  //	Inputs:
  //	- oscSource: Choose either internal 2MHz oscillator or an external
  // signal 		applied to the OSCIO pin.
  //		- INTERNAL_CLOCK and EXTERNAL_CLOCK are defined in the header
  // file. 			Use those.
  ...
  //	- oscFreqOut: If oscio is configured as an output, this will set the
  // output 		frequency
  ...
  void configClock(byte oscSource = 2, byte oscPinFunction = 0,
                   byte oscFreqOut = 0,
                   byte oscDivider = 1);  // Legacy, use clock();

  // -----------------------------------------------------------------------------
  // clock(byte oscSource, byte oscDivider, byte oscPinFunction, byte
  // oscFreqOut)
  //		This function configures the oscillator source/speed
  //		and the clock, which is used to drive LEDs and time debounces.
  ...
  //
  // -----------------------------------------------------------------------------
  void clock(byte oscSource = 2, byte oscDivider = 1, byte oscPinFunction = 0,
             byte oscFreqOut = 0);
};
